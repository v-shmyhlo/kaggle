...
assert config.batch_size % config.samples_in_a_row == 0
...
def compute_loss(input, target, exp):
    assert np.unique(exp).shape[0] == config.batch_size // config.samples_in_a_row, np.unique(exp)
    exp = np.array(exp)
...
def lr_search(train_eval_data):
    ...
    train_eval_data_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True,
        sampler=Sampler(train_eval_dataset.data, config.samples_in_a_row, shuffle=True),
...
def train_fold(fold, train_eval_data):
    ...
    train_data_loader = torch.utils.data.DataLoader(
        ...
        batch_size=config.batch_size,
        drop_last=True,
        sampler=Sampler(train_dataset.data, config.samples_in_a_row, shuffle=True),
        num_workers=args.workers,
        worker_init_fn=worker_init_fn)
    ...
    eval_data_loader = torch.utils.data.DataLoader(
        eval_dataset,
        batch_size=config.batch_size,
        sampler=Sampler(eval_dataset.data, config.samples_in_a_row),
        num_workers=args.workers,
        worker_init_fn=worker_init_fn)
